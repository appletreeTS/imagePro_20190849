...
// ImagePro___20190849View.cpp: CImagePro20190849View 클래스의 구현
//
...
#include "ImagePro___20190849.h"
#endif

#include "ImagePro___20190849Doc.h"
#include "ImagePro___20190849View.h"
...
IMPLEMENT_DYNCREATE(CImagePro20190849View, CScrollView)

BEGIN_MESSAGE_MAP(CImagePro20190849View, CScrollView)
	// 표준 인쇄 명령입니다.
	...
	ON_COMMAND(ID_PIXE_SUB, &CImagePro20190849View::OnPixeSub)
	ON_COMMAND(ID_PIXLE_MUL, &CImagePro20190849View::OnPixleMul)
	ON_COMMAND(ID_PIXEL_DIV, &CImagePro20190849View::OnPixelDiv)
	ON_COMMAND(ID_PIXEL_HISTO_EQU, &CImagePro20190849View::OnPixelHistoEqu)
	ON_COMMAND(ID_PIXEL_CONTRAST_STRETCHING, &CImagePro20190849View::OnPixelContrastStretching)
	ON_COMMAND(ID_PIXEL_BINARIZATION, &CImagePro20190849View::OnPixelBinarization)
	ON_COMMAND(ID_PIXEL_TWO_IMAGE_ADD, &CImagePro20190849View::OnPixelTwoImageAdd)
	ON_COMMAND(ID_PIXEL_TWO_IMAGE_SUB, &CImagePro20190849View::OnPixelTwoImageSub)
	...
	ON_COMMAND(ID_REGION_ROBERTS, &CImagePro20190849View::OnRegionRoberts)
	...
	ON_COMMAND(ID_GEOMETRY_ZOOMIN_PIXEL_COPY, &CImagePro20190849View::OnGeometryZoominPixelCopy)
	ON_COMMAND(ID_GEOMETRY_ZOOMIN, &CImagePro20190849View::OnGeometryZoomin)
	ON_COMMAND(ID_GEOMETRY_ZOOMOUT_SUB, &CImagePro20190849View::OnGeometryZoomoutSub)
	ON_COMMAND(ID_GEOMETRY_MIN_SUB, &CImagePro20190849View::OnGeometryMinSub)
	ON_COMMAND(ID_GEOMETRY_AVG_SAMPLING, &CImagePro20190849View::OnGeometryAvgSampling)
	ON_COMMAND(ID_GEOMETRY_LOTATION, &CImagePro20190849View::OnGeometryLotation)
	ON_COMMAND(ID_GEOMETRY_HOLIZANTAL_FLIP, &CImagePro20190849View::OnGeometryHolizantalFlip)
	ON_COMMAND(ID_GEOMETRY_VERTICAL_FLIP, &CImagePro20190849View::OnGeometryVerticalFlip)
	...
	ON_COMMAND(ID_GEOMETRY_MOBPING, &CImagePro20190849View::OnGeometryMobping)
END_MESSAGE_MAP()
...
void CImagePro20190849View::OnInitialUpdate()
{
...
void CImagePro20190849View::OnPixeSub()
{
	CImagePro20190849Doc* pDoc = GetDocument();
...
void CImagePro20190849View::OnPixleMul()
{
	CImagePro20190849Doc* pDoc = GetDocument();
...
void CImagePro20190849View::OnPixelDiv()
{
	CImagePro20190849Doc* pDoc = GetDocument();
...
void CImagePro20190849View::OnPixelBinarization()
{
	CImagePro20190849Doc* pDoc = GetDocument();
	int thresh = 150;
	int x, y;
	...
	for (y = 0; y < pDoc->imageHeight; y++)
		for (x = 0; x < pDoc->imageWidth; x++)
		{
			...
			else
			{
				...
				if (pDoc->inputImg[y][3 * x + 1] > thresh)
					pDoc->resultImg[y][3 * x + 1] = 255;
				else
					pDoc->resultImg[y][3 * x + 1] = 0;

				if (pDoc->inputImg[y][3 * x + 2] > thresh)
...
void CImagePro20190849View::OnPixelTwoImageSub()
{
	CImagePro20190849Doc* pDoc = GetDocument();
...
int CImagePro20190849View::convolve(unsigned char** inputimg, unsigned char** resultimg, int cols, int rows, float mask[][3], int bias, int depth)
{
	int x, y;
	int i, j;
		...
		for (x = 1; x < cols - 1; x++)
		{
			if (depth == 1)
			{
				...
				for (j = 0; j < 3; j++)
					for (i = 0; i < 3; i++)
					{
						sum += (inputimg[y + j - 1][x + i - 1] * mask[j][i]);
					}
				sum += bias;
				if (sum > 255)	sum = 255;
				else if (sum < 0)	sum = 0;
			...
			else
			{
				...
				for (j = 0; j < 3; j++)
					for (i = 0; i < 3; i++)
					{
						rsum += (inputimg[y + j - 1][3 * (x + i - 1) + 0] * mask[j][i]);
						gsum += (inputimg[y + j - 1][3 * (x + i - 1) + 1] * mask[j][i]);
						bsum += (inputimg[y + j - 1][3 * (x + i - 1) + 2] * mask[j][i]);
					}
				...
				resultimg[y][3 * x + 0] = rsum;
				resultimg[y][3 * x + 1] = gsum;
				resultimg[y][3 * x + 2] = bsum;
			}
		}
	return 0;
}
...
void CImagePro20190849View::OnRegionRoberts()
{
	CImagePro20190849Doc* pDoc = GetDocument();
...
void CImagePro20190849View::OnMonopologyColorToGray()
{
	...
	for (y = 0; y < pDoc->imageHeight; y++)
		for (x = 0; x < pDoc->imageWidth; x++)
		{
			...
			pDoc->resultImg[y][3 * x + 0] = gray;
			pDoc->resultImg[y][3 * x + 1] = gray;
			pDoc->resultImg[y][3 * x + 2] = gray;
		}
...
int CImagePro20190849View::CopyResultToInput()
{
	CImagePro20190849Doc* pDoc = GetDocument();
...
void CImagePro20190849View::OnGeometryZoominPixelCopy()
{
	CImagePro20190849Doc* pDoc = GetDocument();
...
void CImagePro20190849View::OnGeometryZoomin()
{
	CImagePro20190849Doc* pDoc = GetDocument();
...
void CImagePro20190849View::OnGeometryZoomoutSub()
{
	CImagePro20190849Doc* pDoc = GetDocument();
...
void CImagePro20190849View::OnGeometryMinSub()
{
	OnRegionMeaning();
	CopyResultToInput();
...
void CImagePro20190849View::OnGeometryHolizantalFlip()
{
	CImagePro20190849Doc* pDoc = GetDocument();
...
void CImagePro20190849View::OnGeometryVerticalFlip()
{
	CImagePro20190849Doc* pDoc = GetDocument();
...
void CImagePro20190849View::OnGeometryMobping()
{
